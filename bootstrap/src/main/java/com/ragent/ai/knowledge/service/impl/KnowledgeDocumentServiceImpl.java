...
package com.ragent.ai.knowledge.service.impl;
...
import com.ragent.ai.core.chunk.ChunkEmbeddingService;
import com.ragent.ai.core.chunk.ChunkingMode;
import com.ragent.ai.core.chunk.ChunkingOptions;
import com.ragent.ai.core.chunk.ChunkingStrategy;
import com.ragent.ai.core.chunk.ChunkingStrategyFactory;
import com.ragent.ai.core.chunk.VectorChunk;
import com.ragent.ai.core.parser.DocumentParserSelector;
import com.ragent.ai.core.parser.ParserType;
import com.ragent.ai.framework.context.UserContext;
import com.ragent.ai.framework.exception.ClientException;
import com.ragent.ai.framework.mq.producer.MessageQueueProducer;
import com.ragent.ai.ingestion.dao.entity.IngestionPipelineDO;
import com.ragent.ai.ingestion.dao.mapper.IngestionPipelineMapper;
import com.ragent.ai.ingestion.domain.context.IngestionContext;
import com.ragent.ai.ingestion.domain.pipeline.PipelineDefinition;
import com.ragent.ai.ingestion.engine.IngestionEngine;
import com.ragent.ai.ingestion.service.IngestionPipelineService;
import com.ragent.ai.knowledge.config.KnowledgeScheduleProperties;
import com.ragent.ai.knowledge.controller.request.KnowledgeChunkCreateRequest;
import com.ragent.ai.knowledge.controller.request.KnowledgeDocumentPageRequest;
import com.ragent.ai.knowledge.controller.request.KnowledgeDocumentUpdateRequest;
import com.ragent.ai.knowledge.controller.request.KnowledgeDocumentUploadRequest;
import com.ragent.ai.knowledge.controller.vo.KnowledgeChunkVO;
import com.ragent.ai.knowledge.controller.vo.KnowledgeDocumentChunkLogVO;
import com.ragent.ai.knowledge.controller.vo.KnowledgeDocumentSearchVO;
import com.ragent.ai.knowledge.controller.vo.KnowledgeDocumentVO;
import com.ragent.ai.knowledge.dao.entity.KnowledgeBaseDO;
import com.ragent.ai.knowledge.dao.entity.KnowledgeDocumentChunkLogDO;
import com.ragent.ai.knowledge.dao.entity.KnowledgeDocumentDO;
import com.ragent.ai.knowledge.dao.mapper.KnowledgeBaseMapper;
import com.ragent.ai.knowledge.dao.mapper.KnowledgeDocumentChunkLogMapper;
import com.ragent.ai.knowledge.dao.mapper.KnowledgeDocumentMapper;
import com.ragent.ai.knowledge.enums.DocumentStatus;
import com.ragent.ai.knowledge.enums.ProcessMode;
import com.ragent.ai.knowledge.enums.SourceType;
import com.ragent.ai.knowledge.handler.RemoteFileFetcher;
import com.ragent.ai.knowledge.mq.event.KnowledgeDocumentChunkEvent;
import com.ragent.ai.knowledge.schedule.CronScheduleHelper;
import com.ragent.ai.knowledge.service.KnowledgeChunkService;
import com.ragent.ai.knowledge.service.KnowledgeDocumentScheduleService;
import com.ragent.ai.knowledge.service.KnowledgeDocumentService;
import com.ragent.ai.rag.core.vector.VectorSpaceId;
import com.ragent.ai.rag.core.vector.VectorStoreService;
import com.ragent.ai.rag.dto.StoredFileDTO;
import com.ragent.ai.rag.service.FileStorageService;
...
public class KnowledgeDocumentServiceImpl implements KnowledgeDocumentService {
    ...
    private List<VectorChunk> runPipelineProcess(KnowledgeDocumentDO documentDO) {
        ...
        if (pipelineId == null) {
            throw new IllegalStateException("Pipeline 模式下 Pipeline ID 为空：docId=" + docId);
        }
        ...
        if (result.getError() != null) {
            throw new RuntimeException("Pipeline 执行失败：" + result.getError().getMessage(), result.getError());
        }

        List<VectorChunk> chunks = result.getChunks();
        if (chunks == null || chunks.isEmpty()) {
            log.warn("Pipeline 执行完成但未产生分块：docId={}", docId);
            return List.of();
        }
        ...
        if (StringUtils.hasText(requestParam.getProcessMode())) {
            ...
            if (ProcessMode.CHUNK == processMode) {
                ...
                if (!StringUtils.hasText(requestParam.getPipelineId())) {
                    throw new ClientException("使用 Pipeline 模式时，必须指定 Pipeline ID");
                }
                try {
                    ingestionPipelineService.get(requestParam.getPipelineId());
                } catch (Exception e) {
                    throw new ClientException("指定的 Pipeline 不存在: " + requestParam.getPipelineId());
                }
    ...
    private ProcessModeConfig resolveProcessModeConfig(KnowledgeDocumentUploadRequest request) {
        ...
        if (ProcessMode.CHUNK == processMode) {
            ...
            try {
                ...
                throw new ClientException("指定的 Pipeline 不存在: " + request.getPipelineId());
            }
