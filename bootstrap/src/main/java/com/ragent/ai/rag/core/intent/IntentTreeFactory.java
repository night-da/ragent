...
package com.ragent.ai.rag.core.intent;
...
import static com.ragent.ai.rag.enums.IntentLevel.CATEGORY;
import static com.ragent.ai.rag.enums.IntentLevel.DOMAIN;
import static com.ragent.ai.rag.enums.IntentLevel.TOPIC;
...
public class IntentTreeFactory {
    ...
    public static List<IntentNode> buildIntentTree() {
        ...
        IntentNode oaSecurity = IntentNode.builder()
                ...
                .parentId(oa.getId())
                .kind(IntentKind.KB)
                .description("OA 系统的数据权限、访问控制、安全审计等相关说明")
                .examples(List.of(
                        "OA 系统如何控制不同角色的权限？"
                ))
                .build();
    ...
    private static final String MCP_SALES_DATA_PROMPT_TEMPLATE = """
            ...
            【核心处理规则】
            1. **直接回答**：开门见山地回答用户问题，不要使用 "根据数据/JSON 显示" 这类废话作为开头。
            2. **去技术化**：
            ...
            3. **多意图部分匹配**：如果用户问题包含多个子问题，而【动态数据】只能回答其中部分：
               - **先回答能回答的部分**，按正常格式输出数据。
               - **再说明无法回答的部分**，例如："关于『VPN 连接方法』，当前未检索到相关知识，建议咨询 IT 支持。"
