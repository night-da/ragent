...
COMMENT ON COLUMN t_conversation.id IS '主键 ID';
COMMENT ON COLUMN t_conversation.conversation_id IS '会话 ID';
...
COMMENT ON COLUMN t_knowledge_vector.id IS '分块 ID';
...
COMMENT ON COLUMN t_conversation_summary.id IS '主键 ID';
COMMENT ON COLUMN t_conversation_summary.conversation_id IS '会话 ID';
COMMENT ON COLUMN t_conversation_summary.user_id IS '用户 ID';
COMMENT ON COLUMN t_conversation_summary.last_message_id IS '摘要最后消息 ID';
...
-- t_message
COMMENT ON COLUMN t_message.id IS '主键 ID';
COMMENT ON COLUMN t_message.conversation_id IS '会话 ID';
...
-- t_message_feedback
COMMENT ON COLUMN t_message_feedback.id IS '主键 ID';
COMMENT ON COLUMN t_message_feedback.message_id IS '消息 ID';
COMMENT ON COLUMN t_message_feedback.conversation_id IS '会话 ID';
COMMENT ON COLUMN t_message_feedback.user_id IS '用户 ID';
...
COMMENT ON COLUMN t_knowledge_base.collection_name IS 'Collection 名称';
...
COMMENT ON COLUMN t_knowledge_document.kb_id IS '知识库 ID';
...
COMMENT ON COLUMN t_knowledge_document.chunk_config IS '分块配置 JSON';
...
COMMENT ON COLUMN t_knowledge_chunk.kb_id IS '知识库 ID';
COMMENT ON COLUMN t_knowledge_chunk.doc_id IS '文档 ID';
...
COMMENT ON COLUMN t_knowledge_chunk.token_count IS 'Token 数';
...
COMMENT ON COLUMN t_knowledge_document_chunk_log.doc_id IS '文档 ID';
...
COMMENT ON COLUMN t_knowledge_document_chunk_log.persist_duration IS 'DB 持久化耗时（毫秒）';
...
COMMENT ON COLUMN t_knowledge_document_schedule.doc_id IS '文档 ID';
COMMENT ON COLUMN t_knowledge_document_schedule.kb_id IS '知识库 ID';
COMMENT ON COLUMN t_knowledge_document_schedule.cron_expr IS 'Cron 表达式';
...
COMMENT ON COLUMN t_knowledge_document_schedule.last_etag IS '上次 ETag';
...
COMMENT ON COLUMN t_knowledge_document_schedule_exec.schedule_id IS '调度 ID';
COMMENT ON COLUMN t_knowledge_document_schedule_exec.doc_id IS '文档 ID';
COMMENT ON COLUMN t_knowledge_document_schedule_exec.kb_id IS '知识库 ID';
...
COMMENT ON COLUMN t_intent_node.kb_id IS '知识库 ID';
...
COMMENT ON COLUMN t_intent_node.collection_name IS '关联的 Collection 名称';
COMMENT ON COLUMN t_intent_node.top_k IS '知识库检索 TopK';
COMMENT ON COLUMN t_intent_node.mcp_tool_id IS 'MCP 工具 ID';
COMMENT ON COLUMN t_intent_node.kind IS '类型 0：RAG 知识库类 1：SYSTEM 系统交互类';
COMMENT ON COLUMN t_intent_node.prompt_snippet IS '提示词片段';
COMMENT ON COLUMN t_intent_node.prompt_template IS '提示词模板';
COMMENT ON COLUMN t_intent_node.param_prompt_template IS '参数提取提示词模板（MCP 模式专属）';
...
COMMENT ON COLUMN t_rag_trace_run.trace_id IS '全局链路 ID';
COMMENT ON COLUMN t_rag_trace_run.trace_name IS '链路名称';
COMMENT ON COLUMN t_rag_trace_run.entry_method IS '入口方法';
COMMENT ON COLUMN t_rag_trace_run.conversation_id IS '会话 ID';
COMMENT ON COLUMN t_rag_trace_run.task_id IS '任务 ID';
COMMENT ON COLUMN t_rag_trace_run.user_id IS '用户 ID';
...
COMMENT ON COLUMN t_rag_trace_node.trace_id IS '所属链路 ID';
COMMENT ON COLUMN t_rag_trace_node.node_id IS '节点 ID';
COMMENT ON COLUMN t_rag_trace_node.parent_node_id IS '父节点 ID';
...
COMMENT ON COLUMN t_ingestion_pipeline_node.pipeline_id IS '流水线 ID';
...
COMMENT ON COLUMN t_ingestion_pipeline_node.next_node_id IS '下一个节点 ID';
COMMENT ON COLUMN t_ingestion_pipeline_node.settings_json IS '节点配置 JSON';
COMMENT ON COLUMN t_ingestion_pipeline_node.condition_json IS '条件 JSON';
...
COMMENT ON COLUMN t_ingestion_task.pipeline_id IS '流水线 ID';
COMMENT ON COLUMN t_ingestion_task.source_type IS '来源类型';
COMMENT ON COLUMN t_ingestion_task.source_location IS '来源地址或 URL';
...
COMMENT ON COLUMN t_ingestion_task.logs_json IS '节点日志 JSON';
COMMENT ON COLUMN t_ingestion_task.metadata_json IS '扩展元数据 JSON';
...
COMMENT ON COLUMN t_ingestion_task_node.task_id IS '任务 ID';
COMMENT ON COLUMN t_ingestion_task_node.pipeline_id IS '流水线 ID';
...
COMMENT ON COLUMN t_ingestion_task_node.output_json IS '节点输出 JSON(全量)';
