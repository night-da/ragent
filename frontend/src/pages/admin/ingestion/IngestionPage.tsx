...
const pipelineSchema = z.object({
  name: z.string().min(1, "请输入流水线名称").max(60, "名称不能超过 60 个字符"),
...
function PipelineDialog({ open, mode, pipeline, onOpenChange, onSubmit }: PipelineDialogProps) {
  ...
  const buildNodesPayload = (sourceNodes: PipelineNodeForm[]) => {
    ...
    for (const node of sourceNodes) {
      ...
      if (!nodeId) {
        return { ok: false as const, message: "节点 ID 不能为空" };
      }
      if (!node.nodeType) {
    ...
    if (nodeMode === "json") {
      if (values.nodesJson && values.nodesJson.trim()) {
        try {
          ...
          if (!Array.isArray(parsed)) {
            form.setError("nodesJson", { message: "节点配置必须是 JSON 数组" });
            return;
          }
          ...
          form.setError("nodesJson", { message: "节点配置 JSON 格式错误" });
          return;
        }
      }
    } else {
                ...
                {nodes.map((node, index) => (
                  <div key={node.id} className="rounded-lg border p-4 space-y-4">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-medium">下一节点 ID</label>
                        <Input
...
function TaskDialog({ open, pipelineOptions, onOpenChange, onSubmit, onUpload }: TaskDialogProps) {
  ...
  const handleSubmit = async (values: TaskFormValues) => {
    ...
    if (credentials === null) {
      form.setError("credentialsJson", { message: "凭证 JSON 格式错误" });
      return;
    }
    const metadata = parseJsonField(values.metadataJson);
    if (metadata === null) {
      form.setError("metadataJson", { message: "元数据 JSON 格式错误" });
      return;
    }
