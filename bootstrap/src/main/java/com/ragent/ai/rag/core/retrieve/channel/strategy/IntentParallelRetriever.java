...
package com.ragent.ai.rag.core.retrieve.channel.strategy;
...
import com.ragent.ai.rag.core.retrieve.RetrieveRequest;
import com.ragent.ai.rag.core.retrieve.RetrieverService;
import com.ragent.ai.rag.core.retrieve.channel.AbstractParallelRetriever;
...
public class IntentParallelRetriever extends AbstractParallelRetriever<IntentParallelRetriever.IntentTask> {
    ...
    protected List<RetrievedChunk> createRetrievalTask(String question, IntentTask task, int ignoredTopK) {
        ...
        try {
            ...
            log.error("意图检索失败 - 意图 ID: {}, 意图名称: {}, Collection: {}, 错误: {}",
    ...
    protected String getTargetIdentifier(IntentTask task) {
        NodeScore nodeScore = task.nodeScore();
        IntentNode node = nodeScore.getNode();
        return String.format("意图 ID: %s, 意图名称: %s", node.getId(), node.getName());
    }
