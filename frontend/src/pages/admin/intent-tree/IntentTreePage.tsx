...
const formSchema = z.object({
  name: z.string().min(1, "请输入节点名称").max(50, "名称不能超过 50 个字符"),
  intentCode: z
      .string()
      .min(1, "请输入意图标识")
      .max(80, "意图标识过长")
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    if (mode === "create") {
      ...
      if (values.kind === 2 && !values.mcpToolId?.trim()) {
        form.setError("mcpToolId", { message: "请输入 MCP 工具 ID" });
        return;
      }
    } else {
      // 编辑模式下也需要验证 MCP 工具 ID
      if (values.kind === 2 && !values.mcpToolId?.trim()) {
        form.setError("mcpToolId", { message: "MCP 节点必须填写工具 ID" });
        return;
      }
    }
            ...
            <form className="space-y-4" onSubmit={form.handleSubmit(handleSubmit)}>
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="例如：OA 系统" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    )}
              ...
              {kind === 2 && (
                  <FormField
                      ...
                      render={({ field }) => (
                          <FormItem>
                            <FormLabel>MCP 工具 ID（必填）</FormLabel>
                            <FormControl>
                ...
                <div className="mt-3 space-y-4">
                      ...
                      render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Textarea
                                  rows={4}
                                  placeholder="场景用的完整 Prompt 模板，KB 和 MCP 节点都可配置"
                                  {...field}
                              />
                            </FormControl>
                            <FormMessage />
                  ...
                  {kind === 2 && (
                      <FormField
                          ...
                          render={({ field }) => (
                              <FormItem>
                                <FormLabel>参数提取提示词模板（MCP 专属）</FormLabel>
                                <FormControl>
                                  <Textarea
                                      rows={4}
